...
/// Animated avatar that pulses and dances when music is playing.
...
class _ListeningAvatarState extends State<ListeningAvatar>
  ...
  late List<double> _barH;

  @override
  void initState() {
    super.initState();
    _barH = List.generate(4, (_) => 0.3 + _rng.nextDouble() * 0.7);
    ...
    _barCtrl = AnimationController(vsync: this, duration: const Duration(milliseconds: 220))
      ..addListener(() {
        if (mounted) setState(() => _barH = List.generate(4, (_) => 0.2 + _rng.nextDouble() * 0.8));
      });
  ...
  void _stopAnimations() {
    _pulseCtrl.stop(); _bounceCtrl.stop(); _barCtrl.stop();
    _pulseCtrl.animateTo(0); _bounceCtrl.animateTo(0);
  }

  @override
  void dispose() {
    _pulseCtrl.dispose(); _bounceCtrl.dispose(); _barCtrl.dispose();
    super.dispose();
  }

  @override
  Widget build(BuildContext context) {
    final initial = widget.name.isNotEmpty ? widget.name.trim()[0].toUpperCase() : '?';
          ...
          AnimatedBuilder(
            ...
            builder: (_, __) => Transform.translate(
              ...
              child: Transform.scale(
                ...
                child: _AvatarCircle(
                  initial: initial, color: widget.color,
                  size: widget.size, isPlaying: widget.isPlaying,
                ),
              ),
            ),
          ),
...
class _AvatarCircle extends StatelessWidget {
  ...
  final double size;
  final bool   isPlaying;
  const _AvatarCircle({required this.initial, required this.color, required this.size, required this.isPlaying});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Stack(
        ...
        children: [
          Text(initial, style: TextStyle(color: Colors.white, fontSize: size * 0.38, fontWeight: FontWeight.w800, fontFamily: 'Poppins')),
          if (isPlaying)
            Positioned(
              right: 0, bottom: 0,
              child: Container(
                ...
                child: Icon(Icons.music_note_rounded, size: size * 0.16, color: color),
              ),
            ),
        ],
      ),
    ...
    return SizedBox(
      ...
      child: Row(
        ...
        children: bars.map((h) => AnimatedContainer(
          ...
          width: 3, height: (h * 10).clamp(2.0, 10.0),
          margin: const EdgeInsets.symmetric(horizontal: 1),
          decoration: BoxDecoration(borderRadius: BorderRadius.circular(2), color: color.withOpacity(0.85)),
        )).toList(),
      ),
    );
  }
...
/// Picks a vibrant color based on hashing the uid/name string.
Color avatarColorFor(String text) {
  const colors = [
    ...
    Color(0xFFEF4444), Color(0xFF8B5CF6), Color(0xFF10B981), Color(0xFFF97316),
  ];
